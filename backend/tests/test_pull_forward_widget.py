...
class TestPullForwardWidget(unittest.TestCase):
    def test_widget_limits_to_seven_visible_unpaid(self):
        ...
        async def run():
            ...
            def fake_cycle_payments(db, bill_ids, start, end):
                out = {(bill.id, due): row}
                for idx, b in enumerate(extras):
                    d = date(2026, 6, 10 + idx)
            ...
            with patch(
                ...
                "app.services.pay_period_planner.auto_generate_missing_cycle_rows_for_window",
                new_callable=AsyncMock,
            ), patch(
                "app.services.pay_period_planner.get_cycle_payments",
