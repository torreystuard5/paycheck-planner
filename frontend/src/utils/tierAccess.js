...
export function hasBusinessAccess(user, subscription) {
  const accessState = subscription?.access_state;
  if (
    accessState === 'trial_active'
    || accessState === 'early_access'
    || accessState === 'subscribed'
    || accessState === 'bundle'
    || accessState === 'admin_granted'
  ) {
    return true;
  }
  if (subscription?.has_business_access === true) return true;
  ...
  if (subscription?.has_business_access === false) return false;
  return false;
}
