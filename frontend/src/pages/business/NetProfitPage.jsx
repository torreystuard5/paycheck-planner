  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Net profit</h1>
          <p className="text-sm text-gray-600 mt-1">Sales minus deductions and staff pay</p>
        </div>
        <button type="button" onClick={exportCsv} className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50">
        ...
        {range === 'custom' && (
          <>
            <div>
              ...
              <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">End</label>
              <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-8 shadow-sm text-center">
        <p className="text-xs text-gray-500 uppercase tracking-wide mb-2">Net profit</p>
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 text-left max-w-2xl mx-auto text-sm">
          ...
          <div>
            <p className="text-gray-500">Staff pay</p>
            <CurrencyDisplay amount={data?.total_staff_pay} className="font-semibold text-blue-700" />
          </div>
          <div>
            <p className="text-gray-500">Fund deposits</p>
            <CurrencyDisplay amount={data?.total_fund_contributions} className="font-semibold text-gray-800" />
          </div>
        </div>
      </div>

      {chartData.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm h-72">
