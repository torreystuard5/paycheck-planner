...
export default function FundPage() {
  ...
  return (
    <BusinessPageShell
      title={title}
      description="Balance and contributions"
      loading={loading}
      teamRole={teamRole}
      maxWidth="max-w-3xl"
      actions={(
    ...
    >
      {fund ? (
        <div className="bg-surface rounded-lg border border-border p-6 shadow-sm space-y-4">
          <div>
            <p className="text-xs text-muted uppercase">Current balance</p>
            <CurrencyDisplay amount={fund.current_balance} className="text-3xl font-bold text-foreground mt-1 block" />
          </div>
          {fund.target_amount != null && (
            <>
              <div>
                <p className="text-xs text-muted uppercase">Target</p>
                <CurrencyDisplay amount={fund.target_amount} className="text-lg font-medium text-foreground" />
              </div>
              <div>
                <div className="h-3 bg-surface-subtle rounded-full overflow-hidden">
                  <div className="h-full bg-purple-500 rounded-full transition-all" style={{ width: `${pct()}%` }} />
                </div>
                <p className="text-xs text-muted mt-1">{pct()}% of target</p>
              </div>
            </>
          )}
          {fund.monthly_contribution != null && (
            <p className="text-sm text-muted">Monthly contribution: <CurrencyDisplay amount={fund.monthly_contribution} className="inline font-semibold" /></p>
          )}
        </div>
      ) : (
      ...
      <div>
        ...
        {txs.length === 0 ? (
          ...
          <div className="bg-surface border border-border rounded-lg overflow-x-auto shadow-sm">
                ...
                {txs.map((t) => (
                  <tr key={t.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-2">
                      <button type="button" onClick={() => setDelTx(t)} {...write.props({ className: 'p-1 text-muted hover:text-red-600' })}><Trash2 className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal isOpen={txModal} onClose={() => setTxModal(false)} title="Add transaction">
        <form onSubmit={saveTx} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setTxModal(false)} disabled={saving} className="px-4 py-2 text-sm rounded-lg hover:bg-surface-subtle">Cancel</button>
      ...
      <Modal isOpen={editModal} onClose={() => setEditModal(false)} title="Fund settings">
        <form onSubmit={saveEdit} className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-foreground mb-1">Monthly contribution</label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setEditModal(false)} disabled={saving} className="px-4 py-2 text-sm rounded-lg hover:bg-surface-subtle">Cancel</button>
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-purple-600 text-white rounded-lg disabled:opacity-50">{saving ? 'Saving…' : 'Save'}</button>
          </div>
        </form>
      </Modal>
