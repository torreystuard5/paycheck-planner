...
export function WhatsNewUnseenBadge() {
...
function RecentUpdatesPanel({ isExpanded }) {
      ...
      {loading ? (
        ...
        updates.length === 0 ? (
          ...
          <div className="max-h-64 divide-y divide-border overflow-y-auto overscroll-contain">
            {updates.slice(0, 8).map((update, idx) => (
...
/**
 * @param {object} props
 * @param {boolean} [props.embedded] — render panel only (parent supplies CollapsibleCard)
 * @param {boolean} [props.isExpanded] — when embedded, controls lazy fetch
 */
export default function RecentUpdates({ embedded = false, isExpanded = true }) {
  ...
  const panelExpanded = embedded ? isExpanded : !collapsed;

  if (embedded) {
    return <RecentUpdatesPanel isExpanded={panelExpanded} />;
  }
    ...
    <Card className="overflow-hidden" role="region" aria-labelledby="recent-updates-heading">
      ...
      {!collapsed && (
        <div className="border-t border-border">
          <RecentUpdatesPanel isExpanded={panelExpanded} />
        </div>
      )}
    </Card>
  );
