...
class User(Base):
    ...
    # Stripe / subscription (dashboard-managed price IDs)
    ...
    recurring_subscriptions = relationship(
        ...
        foreign_keys="[RecurringSubscription.user_id]",
    )
    user_discounts = relationship(
        "UserDiscount",
        back_populates="user",
        cascade="all, delete-orphan",
        foreign_keys="[UserDiscount.user_id]",
    )
    paystub_uploads = relationship(
        "PaystubUpload",
        back_populates="user",
        cascade="all, delete-orphan",
    )
