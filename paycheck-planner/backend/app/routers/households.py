...
from app.models.household_shopping_item import HouseholdShoppingItem
...
async def list_shopping_items(
    ...
    res = await db.execute(
        ...
        .where(HouseholdShoppingItem.household_id == household.id)
        .order_by(HouseholdShoppingItem.sort_order, HouseholdShoppingItem.created_at)
    )
    items = list(res.scalars().all())
    return ShoppingListResponse(items=[ShoppingItemOut.model_validate(i) for i in items])


@router.post("/shopping-list", response_model=ShoppingItemOut, status_code=status.HTTP_201_CREATED)
...
async def update_shopping_item(
    ...
    household = await _get_household_row(db, current_user)
    res = await db.execute(
        select(HouseholdShoppingItem).where(
            HouseholdShoppingItem.id == item_id,
            HouseholdShoppingItem.household_id == household.id,
        )
    )
    ...
    if body.is_purchased is not None:
        row.is_purchased = body.is_purchased
        if body.is_purchased:
            row.purchased_at = datetime.now(timezone.utc)
            row.purchased_by_user_id = current_user.id
        else:
            row.purchased_at = None
            row.purchased_by_user_id = None
    await db.flush()
    await db.refresh(row)
    return ShoppingItemOut.model_validate(row)
