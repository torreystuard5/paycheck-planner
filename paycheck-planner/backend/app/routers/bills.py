...
async def list_bills(
    ...
    include_hidden: bool = Query(
        default=True,
        description="When false, bills with hidden_overdue are omitted from the list.",
    ),
    ...
    if not include_hidden:
        query = query.where(Bill.hidden_overdue.is_(False))
...
@router.get("/hidden", response_model=list[BillResponse])
async def list_hidden_bills(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Bills marked hidden_overdue (manage from Bills page or dashboard)."""
    if current_user.household_id:
        member_result = await db.execute(
            select(User.id).where(User.household_id == current_user.household_id)
        )
    ...
    query = query.options(selectinload(Bill.assigned_member)).order_by(Bill.updated_at.desc())
    result = await db.execute(query)
    bills = result.scalars().all()
    member_count = await _get_household_member_count(db, current_user.household_id)
    return [_bill_to_response(b, current_user.id, member_count) for b in bills]
