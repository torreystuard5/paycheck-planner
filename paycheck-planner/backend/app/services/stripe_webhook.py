    ...
    if sub_id:
        try:
            ...
            if settings.STRIPE_SECRET_KEY:
                ...
                user.subscription_started_at = _dt_from_ts(getattr(sub, "current_period_start", None))
        except Exception:
...
async def handle_subscription_updated(db: AsyncSession, sub: dict) -> None:
    ...
    user.stripe_subscription_id = str(sub.get("id")) if sub.get("id") else user.stripe_subscription_id
    user.trial_ends_at = _dt_from_ts(sub.get("trial_end"))
    user.subscription_ends_at = _dt_from_ts(sub.get("current_period_end"))
    user.subscription_started_at = _dt_from_ts(sub.get("current_period_start")) or user.subscription_started_at
