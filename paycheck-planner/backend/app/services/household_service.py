...
async def resolve_valid_household_id(db: AsyncSession, user: User) -> UUID | None:
    """Return ``user.household_id`` only if that household row exists.

    Avoids FK violations when ``users.household_id`` is stale (e.g. household
    removed while the column was not cleared).
    """
    if user.household_id is None:
        return None
    result = await db.execute(
        select(Household.id).where(Household.id == user.household_id)
    )
