...
export default function Bills({ autoOpenAdd, onClearAutoOpen }) {
  ...
  const [includeHiddenBills, setIncludeHiddenBills] = useState(false);
  ...
  useEffect(() => {
    fetchBills(true);
    fetchPaycheckDates();
  }, [statusFilter, sortBy, sortOrder, includeHiddenBills]);
  ...
  const pollBills = useCallback(async () => {
    try {
      ...
      queryParams.set('include_hidden', includeHiddenBills ? 'true' : 'false');
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600 whitespace-nowrap shrink-0 cursor-pointer">
            <input
              type="checkbox"
              checked={includeHiddenBills}
              onChange={(e) => setIncludeHiddenBills(e.target.checked)}
