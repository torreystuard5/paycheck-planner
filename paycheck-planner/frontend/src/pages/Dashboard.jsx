...
import { formatDate, formatFriendlyDate, formatPaycheckDate } from '../utils/formatDate';
...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [planRes, creditRes, upcomingRes] = await Promise.allSettled([
        api.get('/api/v1/paycheck-plan'),
        api.get('/api/v1/debts/credit-efficiency'),
        api.get('/api/v1/dashboard/upcoming-bills', { params: { days: 21, limit: 12 } }),
      ]);
      ...
      if (upcomingRes.status === 'fulfilled') {
        setUpcomingBills(Array.isArray(upcomingRes.value.data) ? upcomingRes.value.data : []);
      } else {
        setUpcomingBills([]);
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {paycheckPlan?.paychecks?.[0] && (
        <div className="bg-gradient-to-r from-emerald-50 to-white rounded-lg border border-emerald-100 p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="text-xs font-medium text-emerald-800 uppercase tracking-wide">This pay period</p>
            <p className="text-lg font-semibold text-gray-900 mt-0.5">Remaining to spend</p>
            <p className="text-sm text-gray-500 mt-1">After assigned bills and debts for your next paycheck.</p>
          </div>
          <div className="text-right">
            <CurrencyDisplay
              amount={paycheckPlan.paychecks[0].remaining}
              className={`text-2xl font-bold ${Number(paycheckPlan.paychecks[0].remaining) >= 0 ? 'text-emerald-700' : 'text-red-600'}`}
            />
            <p className="text-xs text-gray-500 mt-1">
              Paycheck {formatPaycheckDate(paycheckPlan.paychecks[0].paycheck_date)}
            </p>
          </div>
        </div>
      )}
      ...
      {upcomingBills.length > 0 && (
        ...
        >
          <p className="text-sm text-gray-500 mb-3">Unpaid bills due in the next few weeks.</p>
          <ul className="divide-y divide-gray-100 border border-gray-100 rounded-lg overflow-hidden">
            {upcomingBills.map((b) => (
              <li key={b.id} className="flex items-center justify-between gap-3 px-4 py-2.5 bg-white hover:bg-gray-50">
                <Link to="/bills-debts?tab=bills" className="text-sm font-medium text-gray-900 truncate min-w-0 hover:text-blue-600">
                  {b.name || 'Bill'}
                </Link>
                <div className="shrink-0 text-right">
                  <CurrencyDisplay amount={b.amount} className="text-sm font-semibold text-gray-900" />
                  <p className="text-xs text-gray-500">
                    {b.next_due_date ? formatFriendlyDate(b.next_due_date) : '—'}
                  </p>
                </div>
              </li>
            ))}
