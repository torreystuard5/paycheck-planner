...
import { Users, UserPlus, Copy, LogOut, Activity, Clock, Settings, CheckCircle, DollarSign, ClipboardList, Trash2, ShoppingBag } from 'lucide-react';
...
export default function Household() {
  ...
  }, [fetchHousehold, fetchActivity, fetchHouseholdBills, fetchChores, fetchShopping]);
  ...
  const handleAddShopping = async (e) => {
    ...
    setError(null);
    try {
      await api.post('/api/v1/households/shopping-list', { label: shoppingLabel.trim() });
      setShoppingLabel('');
      await fetchShopping();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <p className="text-sm text-gray-500 mb-4">Shared with everyone in your household. Check items off when you buy them.</p>
        <form onSubmit={handleAddShopping} className="flex flex-col sm:flex-row gap-2 mb-4">
          <input
          ...
          <button
            type="submit"
            disabled={shoppingBusy || !shoppingLabel.trim()}
        ...
        {shoppingItems.length === 0 ? (
          ...
          <ul className="space-y-2">
            {shoppingItems.map((item) => (
              <li
                key={item.id}
                className={`flex items-center justify-between gap-2 border border-gray-100 rounded-lg px-3 py-2 text-sm ${item.is_purchased ? 'bg-gray-50 opacity-70' : ''}`}
              >
                <button
                  type="button"
                  onClick={() => handleToggleShopping(item)}
                  className={`text-left flex-1 min-w-0 ${item.is_purchased ? 'line-through text-gray-500' : 'text-gray-900 font-medium'}`}
                >
                  {item.label}
                </button>
                <button
