...
const TABS = ['Monthly Summary', 'Trends', 'Spending', 'Interest'];
...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get('/api/v1/reports/spending', { params: { months: 8 } });
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'Spending' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Spending by month</h2>
            {!spendingReport?.by_month?.length ? (
              <EmptyState icon={BarChart3} title="No payment history" message="Log payments to see spending over time." />
            ) : (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={spendingReport.by_month.map((r) => ({ ...r, total: toSafeNumber(r.total) }))}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="month" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} />
                  <Tooltip formatter={(value) => [`$${toSafeNumber(value).toFixed(2)}`, 'Paid']} />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {!spendingReport?.by_category?.length ? (
              <p className="text-sm text-gray-500">No categorized payments yet.</p>
            ) : (
              <div className="flex flex-col lg:flex-row items-center gap-8">
                <ResponsiveContainer width="100%" height={280}>
                  <PieChart>
                    <Pie
                      data={spendingReport.by_category.map((r) => ({ name: r.category, value: toSafeNumber(r.total) }))}
                      cx="50%"
                      cy="50%"
                      outerRadius={100}
                      dataKey="value"
                    >
                      {spendingReport.by_category.map((_, idx) => (
                        <Cell key={idx} fill={COLORS[idx % COLORS.length]} />
                      ))}
                    </Pie>
                ...
                <div className="w-full lg:w-64 space-y-2">
                  {spendingReport.by_category.map((row, idx) => (
                    <div key={row.category} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                        <span className="text-gray-700 truncate">{row.category}</span>
                      </div>
                      <span className="font-medium text-gray-900 shrink-0">${toSafeNumber(row.total).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
